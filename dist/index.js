...
async function run() {
    try {
        const token = (0, core_1.getInput)('token') || process.env.TOKEN;
        const owner = (0, core_1.getInput)('repository').split('/')[0] ||
            process.env.GITHUB_REPOSITORY?.split('/')[0];
        const repo = (0, core_1.getInput)('repository').split('/')[1] ||
        ...
        const lastStoredRedeliveryTime = await getVariable(lastRedeliveryVariable, owner, repo, octokit);
        (0, core_1.debug)(`lastStoredRedeliveryTime: ${lastStoredRedeliveryTime}`);
        const lastWebhookRedeliveryTime = Number(lastStoredRedeliveryTime) || Date.now() - 24 * 60 * 60 * 1000;
        for (const webhook of webhooks) {
            const deliveries = await fetchWebhookDeliveriesSince(lastWebhookRedeliveryTime, owner, repo, webhook.id, octokit);
            const deliveriesByGuid = {};
            ...
            for (const guid in deliveriesByGuid) {
                const deliveriesForGuid = deliveriesByGuid[guid];
                if (deliveriesForGuid.some((delivery) => delivery.status === 'OK')) {
                    break;
                }
                (0, core_1.info)(`Redelivering webhook delivery ${deliveriesForGuid[0]?.id} for webhook ${webhook.id}`);
        ...
        await updateVariable(lastRedeliveryVariable, Date.now().toString(), Boolean(lastStoredRedeliveryTime), owner, repo, octokit);
    }
    catch (error) {
...
exports.run = run;
async function fetchWebhookDeliveriesSince(lastWebhookRedeliveryTime, repoOwner, repoName, hookId, octokit) {
    const iterator = octokit.paginate.iterator(octokit.rest.repos.listWebhookDeliveries, {
        owner: repoOwner,
        repo: repoName,
        hook_id: hookId,
        per_page: 100,
